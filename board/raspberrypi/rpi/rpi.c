...
int ft_board_setup(void *blob, struct bd_info *bd)
{
	...
	/*
	 * Enable MIP MSI controllers in the OS FDT.
	 * The VPU firmware ships mip0/mip1 with status="disabled" (sometimes
	 * using the downstream "brcm,bcm2712-mip-intc" compatible instead of
	 * the upstream "brcm,bcm2712-mip").  pcie2 declares
	 * msi-parent = <&mip0>, so without an active MIP MSI domain the
	 * rp1_pci driver fails with -ENOTSUPP on pci_alloc_irq_vectors(),
	 * which prevents RP1 (ethernet, USB, GPIO) from probing at all.
	 */
	{
		static const char * const mip_compats[] = {
			"brcm,bcm2712-mip",
			"brcm,bcm2712-mip-intc",
		};
		int i, offset, ret;

		for (i = 0; i < ARRAY_SIZE(mip_compats); i++) {
			offset = -1;
			while (true) {
				const char *st;
				...
				if (offset < 0)
					break;
				st = fdt_getprop(blob, offset, "status", NULL);
				if (!st || strcmp(st, "disabled") != 0)
					continue;
				ret = fdt_setprop_string(blob, offset, "status", "okay");
				if (ret)
					log_warning("ft_board_setup: MIP enable failed: %d\n",
						    ret);
				else
					log_info("ft_board_setup: enabled MIP MSI node (%s)\n",
						 mip_compats[i]);
			}
		}
	}
